...
ArgOption = namedtuple(
    "ArgOption", ["cli_name", "init_name", "pass_value", "fail_value"]
)
ARGUMENT_SPEC_TYPED = [
    ArgOption(cli_name="width", init_name="width", pass_value=13, fail_value=1.0),
    ArgOption(cli_name="height", init_name="height", pass_value=15, fail_value=1.0),
    ArgOption(cli_name="margin", init_name="margin", pass_value=17, fail_value=1.0),
    ArgOption(
        cli_name="relative_scaling",
        init_name="relative_scaling",
        pass_value=1,
        fail_value="c",
    ),
]
ARGUMENT_SPEC_UNARY = [
    ArgOption(
        cli_name="no_collocations",
        init_name="collocations",
        pass_value=True,
        fail_value=1,
    ),
    ArgOption(
        cli_name="include_numbers",
        init_name="include_numbers",
        pass_value=True,
        fail_value=2,
    ),
    ArgOption(
        cli_name="no_normalize_plurals",
        init_name="normalize_plurals",
        pass_value=True,
        fail_value=3,
    ),
...
ARGUMENT_SPEC_REMAINING = [
    ...
    ArgOption(
        cli_name="regexp", init_name="regexp", pass_value=r"\w{2,}", fail_value=r"12("
    ),
    ArgOption(
        cli_name="mask", init_name="mask", pass_value=PassFile(), fail_value=None
    ),
    ArgOption(
        cli_name="fontfile",
        init_name="font_path",
    ...
    ArgOption(
        cli_name="color", init_name="color_func", pass_value="red", fail_value=None
    ),
    ArgOption(
        cli_name="background",
        init_name="background_color",
        pass_value="grey",
        fail_value=None,
    ),
    ArgOption(
        cli_name="contour_color",
        init_name="contour_color",
        pass_value="grey",
        fail_value=None,
    ),
    ArgOption(
        cli_name="contour_width",
        init_name="contour_width",
        pass_value=0.5,
        fail_value="blue",
    ),
    ArgOption(
        cli_name="min_word_length",
        init_name="min_word_length",
        pass_value=5,
        fail_value="blue",
    ),
    ArgOption(
        cli_name="prefer_horizontal",
        init_name="prefer_horizontal",
        pass_value=0.1,
        fail_value="blue",
    ),
    ArgOption(cli_name="scale", init_name="scale", pass_value=1.0, fail_value="blue"),
    ArgOption(
        cli_name="colormap", init_name="colormap", pass_value="Greens", fail_value=1
    ),
    ArgOption(cli_name="mode", init_name="mode", pass_value="RGBA", fail_value=2),
    ArgOption(
        cli_name="max_words", init_name="max_words", pass_value=10, fail_value="blue"
    ),
    ArgOption(
        cli_name="min_font_size",
        init_name="min_font_size",
        pass_value=10,
        fail_value="blue",
    ),
    ArgOption(
        cli_name="max_font_size",
        init_name="max_font_size",
        pass_value=10,
        fail_value="blue",
    ),
    ArgOption(
        cli_name="font_step", init_name="font_step", pass_value=10, fail_value="blue"
    ),
    ArgOption(
        cli_name="random_state",
        init_name="random_state",
        pass_value=100,
        fail_value="blue",
    ),
]
...
def test_main_passes_arguments_through(tmpdir):
    ...
    with patch(
        "wordcloud.wordcloud_cli.wc.WordCloud", autospec=True
    ) as mock_word_cloud:
...
def check_argument(text_filepath, name, result_name, value):
    args, text, image_file = cli.parse_args(
        ["--text", text_filepath, "--" + name, str(value)]
    )
...
def test_parse_args_are_passed_along(option, tmpdir, tmp_text_file):
    if option.cli_name in ARGUMENT_CLI_NAMES_UNARY:
        ...
        check_argument(
            str(tmp_text_file), option.cli_name, option.init_name, pass_value
        )
...
def test_cli_writes_to_imagefile(tmpdir, tmp_text_file):
    ...
    args, text, image_file = cli.parse_args(
        ["--text", str(tmp_text_file), "--imagefile", str(tmp_image_file)]
    )
...
def test_cli_as_executable(
    command, expected_output, expected_exit_code, tmpdir, capfd, no_cover_compat
):

    ret_code = 0
    try:
        subprocess.check_call(command, shell=True, cwd=str(tmpdir))
